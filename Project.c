	...
	while(pList != NULL && found == 0)
	{
		if((pList->category == ctg) && (pList->productID == prodID))
		{
			...
			do
			{
				...
				else if((pList->cost * qty) > cList->balance)
				{
					printf("User does not have enough money in their account. \n");
					rep = 1;
				}
				else
				{
					...
					cList->balance = cList->balance - price;
...
StatusCode returnProduct(struct Product* pList, struct Consumer* cList)
{
	StatusCode sc = SUCCESS;
	...
	while(pList != NULL && found == 0)
	{
		if((pList->category == ctg) && (pList->productID == prodID))
		{
			...
			pList->qty += qty;
			float price;
			price =  qty * pList->cost;
			cList->balance = cList->balance + price;
			printf("Amount Refunded: %.1f \n",price);
			printf("Updated Balance: %.1f \n",cList->balance);
		}
		else
			pList = pList->nextPr;
	}
...
int main()
{
	...
	while(!feof(fp))
	{
		...
		fscanf(fp,"%d ",&qty);
		fscanf(fp,"%d ",&ctg);
		addProduct(&itemsList,consID,ItemName,cost,qty,ctg);
	}
	fclose(fp);
			...
			do
			{
				...
				switch(choice)
				{
					...
					case 3: //Return a product - works
						{
							StatusCode sc;
							sc = returnProduct(itemsList, customersList);	
							//O/P
							break;	
						}
